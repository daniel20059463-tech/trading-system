    ...
    try:
        ...
        for ticker in ALL_STOCKS:
            try:
                ...
                print(f"  [跳過] {ticker}: {e}")
        if ok != len(ALL_STOCKS):
            raise RuntimeError(f"僅 {ok}/{len(ALL_STOCKS)} 支更新成功")
        from data.fetch_stocks import validate_pre_market_data
        asof = validate_pre_market_data(ALL_STOCKS)
        print(f"  資料更新完成：{ok}/{len(ALL_STOCKS)} 支，共同截止日 {asof}")
    except Exception as e:
        raise RuntimeError(f"盤前資料更新／校驗失敗，停止發布：{e}") from e
