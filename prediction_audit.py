...
def freeze(predictions, kind="point", root=HERE, now=None):
    ...
    for original in predictions:
        ...
        try:
            asof_day = datetime.fromisoformat(asof).date()
        except (TypeError, ValueError) as error:
            raise ValueError(f"{p.get('ticker')} 資料截止日格式錯誤：{asof}") from error
        if asof_day.weekday() >= 5:
            raise ValueError(f"{p.get('ticker')} 資料截止日落在週末：{asof}")
        history = prices(root, p.get("ticker", ""))
        valid_days = sorted(d for d in history if d < target and datetime.fromisoformat(d).weekday() < 5)
        expected = valid_days[-1] if valid_days else None
        base = number(p.get("last_close"))
        expected_close = number(history.get(expected))
        price_mismatch = (base is None or expected_close is None or
                          abs(base - expected_close) > max(0.02, expected_close * 0.0001))
        if asof != expected or price_mismatch:
            raise ValueError(f"{p.get('ticker')} 預測基準與最新完整收盤不一致")
        asof_dates.add(asof)
        p.update(target_date=target, generated_at=now.isoformat(), horizon_sessions=1)
        records.append(p)
    if len(asof_dates) != 1:
        raise ValueError("同批預測的資料截止日不一致")
    if not records:
