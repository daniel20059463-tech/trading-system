...
class TradingOrchestrator:
    ...
    def run_pre_market(self) -> dict:
        ...
        # 強利多方向修正（Fable 稽核 F3：盤前發布的預測須與盤後驗證同一套規則，
        # 否則成績單衡量的是「從未發布過的策略」）。新聞 JSON 此刻已由 Step 3 寫出。
        try:
            from strong_news_override import adjust_direction
            n_adj = 0
            for p in predictions:
                code = str(p.get("ticker", "")).replace(".TWO", "").replace(".TW", "")
                pct = p.get("predicted_change_pct")
                if pct is None:
                    continue
                new_pct, trig, note = adjust_direction(code, float(pct), self.today_str)
                if trig:
                    base = p.get("last_close") or p.get("prev_close")
                    p["predicted_change_pct"] = new_pct
                    if base:
                        p["predicted_close"] = round(float(base) * (1 + new_pct / 100), 2)
                    p["direction"] = "UP"
                    p["strong_news"] = note
                    n_adj += 1
            if n_adj:
                print(f"      強利多方向修正：{n_adj} 支盤前預測已同步翻多（與盤後驗證一致）")
        except Exception as e:
            logger.debug(f"強利多盤前修正失敗：{e}")
        ...
        # 盤前推播到 Discord：隔日區間預測 +（如有）regime 警告。地板僅快取供盤中用，不推訊息。
        try:
            ...
            if WEBHOOK:
                ...
                # 推播隔日區間預測（50%最可能/80%風險），亂市以區間為主
                try:
                    from push_intervals import push_to_discord
                    if push_to_discord():
                        print("      已推播隔日區間預測到 Discord")
                except Exception as pe:
                    logger.debug(f"區間推播失敗：{pe}")
                if regime_tip:
